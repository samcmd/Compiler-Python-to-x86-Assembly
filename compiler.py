"""
Added convert to closures to compiler passes.
Convert to closures needs its implementation finished.
Need to implement lexical scope in the typechecker.
Need to add variable case: Assign(x, Var(y)) when y is a function to select instructions.
"""
...
def convert_to_closures(prog: Program) -> Program:
    ...
    def cc_stmt(stmt: Stmt, bindings: Dict[str, Expr]) -> Stmt:
        match stmt:
            case FunctionDef(name, params, body_stmts, return_type):
                return FunctionDef(name, params, cc_stmts(body_stmts), return_type)
            case Return(e):
                return Return(cc_exp(e, bindings))
            case Assign(x, e1):
                new_e1 = cc_exp(e1, bindings)
            ...
            case While(condition, body_stmts):
                condition_bindings = {}
                condition_exp = cc_exp(condition, condition_bindings)
            ...
            case If(condition, then_stmts, else_stmts):
                new_condition = cc_exp(condition, bindings)
                new_then_stmts = cc_stmts(then_stmts)
                new_else_stmts = cc_stmts(else_stmts)

                return If(new_condition,
                          new_then_stmts,
                          new_else_stmts)
            case _:
                raise Exception('cc_stmt', stmt)
    ...
    def cc_stmts(stmts: List[Stmt]) -> List[Stmt]:
        ...
        for stmt in stmts:
            ...
            new_stmt = cc_stmt(stmt, bindings)
            # (2) add the new bindings created by cc_exp
    ...
    def cc_exp(e: Expr, bindings: Dict[str, Expr]) -> Expr:
        match e:
            case Var(x):
            ...
            case Constant(i):
                return Constant(i)
            case Prim(op, args):
                new_args = [cc_exp(e, bindings) for e in args]
            ...
            case Call(func, args):
                new_args = [cc_exp(e, bindings) for e in args]
                new_func = cc_exp(func, bindings)
                new_e = Call(new_func, new_args)
                new_v = gensym('tmp')
                bindings[new_v] = new_e
                return Var(new_v)
            case _:
                raise Exception('cc_exp', e)
...
compiler_passes = {
    ...
    'convert to closures': convert_to_closures,
